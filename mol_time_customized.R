...
mol_time <- function(res.dir="",samples=NA,data.dir="data",grey.thresh=0.2,hclust.thresh=0.4,seed=3,iter=1000,warn=F,eps=F) {
  ...
  for(j in (1:length(sample_list)))  
  {
    ...
    { 
      ...
      #first$class_sh<- chr_segment_list[i]   #### shaghayegh
      ...
      if(unique(first$class) == "LOH") ###### single gain
         { myMclust <- Mclust(myData,G=2,verbose=FALSE)
            ...
            { myMclust <- Mclust(myData,G=2:9,verbose=FALSE) 
            }
    ...
    if (unique(first_def$class) == "gain_3" & length(number_clust)<=4 & length(number_clust)>3 ){
     ...
     if (unique(first_def$class) == "gain_3" & length(number_clust)<3) { mol_time_warn(warn,"no mclust_mm %s",sample_code)
                 next  }  
    
    if (unique(first_def$class) == "LOH" & length(number_clust)<=2 ){
...
all_code_final_comb <- mol_time_ic_gen(all_code,iter=iter)
    ...
    }  else { mol_time_warn(warn,"no mclust_mm %s",sample_code)
                 next  }  
 }  # for j
...
###### generate molecular time and IC for gain
  ...
  bs.code = bs.code[!is.na(bs.code)]   #### Added by Shaghayegh ####
  bs.chrm = bs.chrm[!is.na(bs.chrm)]   #### Added by Shaghayegh ####
  bs.segm = bs.segm[!is.na(bs.segm)]   #### Added by Shaghayegh ####
  bs.cnvn = bs.cnvn[!is.na(bs.cnvn)]   #### Added by Shaghayegh ####
  bs.chvn = bs.chvn[!is.na(bs.chvn)]   #### Added by Shaghayegh ####
